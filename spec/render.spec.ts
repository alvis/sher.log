...
describe('fn:renderError', () => {
  ...
  it('should keep the full parent stack when no frame is shared with the cause', async () => {
    const cause = new Error('inner');
    cause.stack = 'Error: inner\n    at deep (unrelated:1:0)';

    const error = new Xception('outer', { cause });
    error.stack = 'Xception: outer\n    at entry1 (src1:1:0)';

    const rendered = (await renderError(error)).replace(ansi, '');

    // the parent must not lose its own frame just because it shares nothing with the cause
    expect(rendered).toContain(
      '[Xception] outer\n' + '    at entry1 (src1:1:0)',
    );
    expect(rendered).toContain(
